...
//$sendd=$take_clicks+1;
	...
	$result=mysqli_query($database,"Select No_of_clicks from jam");
	$row=mysqli_fetch_assoc($result);
	$take_clicks=(int)$row['No_of_clicks'];
	$sendd=$take_clicks+1;
	$updating="UPDATE `jam` SET `No_of_clicks` = '$sendd' WHERE `id` = '1'";
...
if($database->query($updating))
{
	...
	header("location:https://meet.google.com/lookup/Jam".$rounding);
//	$link=;
