...
//$sendd=$take_clicks+1;
	...
	$User_click=$_POST['click'];
	
	//$data="Select No_of_clicks from gd";  
	
	
	$result=mysqli_query($database,"Select No_of_clicks from clicks_table");
	$row=mysqli_fetch_assoc($result);
	$take_clicks=(int)$row['No_of_clicks'];
	$sendd=$take_clicks+1;
	$updating="UPDATE `clicks_table` SET `No_of_clicks` = '$sendd' WHERE `id` = '1'";
...
if($database->query($updating))
{
	...
	header("location:https://meet.google.com/lookup/debate".$rounding);
//	$link=;
